...
/// Handle to control the stats logger background task.
/// Sending a signal via `shutdown()` will stop the stats logger loop.
pub struct StatsLoggerHandle {
    cancel_token: CancellationToken,
}

impl StatsLoggerHandle {
    /// Signal the stats logger to stop.
...
pub fn spawn_stats_logger(
        ...
        loop {
            tokio::select! {
                ...
                _ = task_token.cancelled() => {
                    tracing::debug!("stats logger received shutdown signal, exiting");
                    break;
                }
            }
        }
    ...
    StatsLoggerHandle { cancel_token }
}
