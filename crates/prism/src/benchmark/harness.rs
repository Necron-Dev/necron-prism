...
mod imp {
    ...
    impl DrainState {
        ...
        fn record(&self, bytes: u64) {
            self.observed_bytes.fetch_add(bytes, Ordering::Relaxed);
        ...
        fn wait_for(&self, target: u64) {
            ...
            while self.observed_bytes.load(Ordering::Relaxed) < target {
                self.notify_cv.wait(&mut guard);
            }
        }
    }

    pub struct RelayHarness {
        client: std::cell::RefCell<TcpStream>,
